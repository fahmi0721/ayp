...
{
    private $table = "m_desa";
    private $pk_table = "";
    public function index(){
        $data = DB::table($this->table)
                ->join("m_kabupaten",$this->table.".id_kabupaten" ,"=","m_kabupaten.id")
                ->join("m_kecamatan",$this->table.".id_kecamatan" ,"=","m_kecamatan.id")
                ->select($this->table.".*",DB::raw("m_kabupaten.nama as kabupaten"),DB::raw("m_kecamatan.nama as kecamatan"))
                ->get();
        return Datatables::of($data)
        ->addIndexColumn()
        ->addColumn('action', function($row){
            $btn = '<div class="btn-group"><a href="'.url("admin/desa/edit").'/'.base64_encode($row->id).'" class="btn btn-primary btn-xs"><i class="fa fa-xs fa-edit"></i></a>';
    ...
    public function index_kabupaten($id_kab){
        ...
        ->addIndexColumn()
        ->addColumn('action', function($row){
            $btn = '<div class="btn-group"><a href="'.url("kabupaten/desa/edit").'/'.base64_encode($row->id).'" class="btn btn-primary btn-xs"><i class="fa fa-xs fa-edit"></i></a>';
            $btn .= '<a href="javascript:void(0)" onclick=\'hapus_data("'.base64_encode($row->id).'")\' class="btn btn-danger btn-xs"><i class="fa fa-xs fa-trash"></i></a></div>';
    ...
    public function index_kecamatan($id_kec){
        ...
        ->addIndexColumn()
        ->addColumn('action', function($row){
            $btn = '<div class="btn-group"><a href="'.url("kecamatan/desa/edit").'/'.base64_encode($row->id).'" class="btn btn-primary btn-xs"><i class="fa fa-xs fa-edit"></i></a>';
            $btn .= '<a href="javascript:void(0)" onclick=\'hapus_data("'.base64_encode($row->id).'")\' class="btn btn-danger btn-xs"><i class="fa fa-xs fa-trash"></i></a></div>';
    ...
    public function show_by_kec($id_kec){
        $id_kecamatan = base64_decode($id_kec);
        $res = DB::table($this->table)->select("id",DB::raw("nama as text"))->where("id_kecamatan",$id_kecamatan)->get();
